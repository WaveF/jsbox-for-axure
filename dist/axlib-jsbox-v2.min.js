...
(function() {
  ...
  function main2() {
    ...
    window.AXLIB = window.axlib = {
      ...
      // 通过元件名获取元素
      loadOnce,
      // 对指定资源只加载一次，并通过lock参数阻止其他脚本重复加载资源，然后通过check参数让其他脚本在资源准备好时访问
      plugins: getPlugins(),
  ...
  function InitAxlib() {
    ...
    $axure.internal(function($ax) {
      ...
      $axure.ex.formatRepeaterData = function(data, options) {
        ...
        if (format === "default") {
          ...
          for (let i = 0; i < data.length; i++) {
            let item2 = data[i];
            let obj = {};
            let key = Object.values(item2)[0].text;
            let val = [];
            for (let k in item2) {
              let sub = item2[k];
              if (sub.text != key) {
          ...
          if (!options.key) {
            ...
            for (let i = 0; i < data.length; i++) {
              let item2 = data[i];
              let key = item2[pKey].text;
              let val = [];
              let obj = {};
              for (let k in item2) {
                if (k != pKey) {
                  val.push(item2[k].text);
                }
              }
              if (options.first) {
                val = val[0];
      ...
      $axure.ex.addRepeaterData = function(elementId, rows) {
        ...
        for (let i = 0, ilen = rows.length; i < ilen; i++) {
          ...
          for (let j = 0, jlen = columns.length; j < jlen; j++) {
            ...
            let item2 = source[column];
            if (item2 === void 0) {
              item2 = itemDefaultValue;
            } else if (!item2.hasOwnProperty("type") && !item2.hasOwnProperty("text")) {
              item2 = { type: "text", text: item2 };
            } else {
              item2 = $ax.deepCopy(item2);
            }
            target[column] = item2;
          }
      ...
      $ax.public.fn.getOverlapData = function(extraRawData2, format) {
        let elementId = this.getElementIds()[0];
        let rep2 = $axure(`#${elementId}`);
        let latestRawData = rep2.getRepeaterData({ format: "default" });
        let overlapData = getOverlapData(extraRawData2, latestRawData);
      ...
      $axure.ex.getCurrentPanelState = function(elementId) {
        ...
        $.each(states, (idx2, item2) => {
          if ($(item2).css("visibility") != "hidden" && $(item2).css("display") != "none") {
            current = item2;
          }
        });
        return current;
  ...
  function getDiffData(data1, data2) {
    let diff = data1.concat(data2).map((item2) => JSON.stringify(item2)).filter((value, idx2, arr) => {
  ...
  function tree(_data) {
    ...
    for (let i = 0; i < maxLevel; i++) {
      ...
      for (let k in _data) {
        let item2 = _data[k];
        arr.push(item2[cid]);
        obj.push({ value: item2[cid], label: item2[cid], parent: item2[`c${i}`], children: [] });
      }
      unique(arr, obj)[0];
  ...
  function trimTreeData(arr) {
    for (let i = 0; i < arr.length; i++) {
      ...
      if (item2.children.length > 0) {
        if (item2.children.length == 1 && !item2.children[0].value) {
          delete item2.children;
        } else {
          trimTreeData(item2.children);
        }
  ...
  function loadJsModule(url, callback) {
    ...
    script2.onload = callback || function() {
    };
    script2.type = "module";
    script2.src = url;
  ...
  function initYepNope() {
    ...
    function injectJs(src, cb, attrs, timeout, err, internal) {
      var script2 = doc.createElement("script"), done, i;
      timeout = timeout || yepnope2["errorTimeout"];
      script2.src = src;
      for (i in attrs) {
        script2.setAttribute(i, attrs[i]);
      }
      cb = internal ? executeStack : cb || noop;
      script2.onreadystatechange = script2.onload = function() {
        if (!done && isFileReady(script2.readyState)) {
          done = 1;
          cb();
      ...
      err ? script2.onload() : firstScript.parentNode.insertBefore(script2, firstScript);
    }
    ...
    function executeStack() {
      ...
      if (i) {
        if (i["t"]) {
          sTimeout(function() {
            (i["t"] == "c" ? yepnope2["injectCss"] : yepnope2["injectJs"])(i["s"], 0, i["a"], i["x"], i["e"], 1);
          }, 0);
        } else {
          i();
          executeStack();
    ...
    yepnope2["addPrefix"] = function(prefix, callback) {
      prefixes[prefix] = callback;
    };
    yepnope2["addFilter"] = function(filter) {
      globalFilters.push(filter);
    };
    yepnope2["errorTimeout"] = 1e4;
...
(function() {
  ...
  function inited() {
    ...
    let repeaterIds = [];
    $.each($jsboxes, (idx2, item2) => {
      repeaterIds.push($(item2).parent().parent().id());
    });
    ...
    $.each(repeaterIds, (idx, item) => {
      ...
      rep.getRepeaterDataOnce({ format: "default" });
      ...
      let getUserElements = function() {
        ...
        $.each(wrapper.children(), (idx2, item2) => {
          if (!item2.className.includes("ax-jsbox")) {
            _els.push(item2);
          }
        });
        return _els;
      ...
      let getNode = function(el) {
        ...
        if (el.$ !== void 0) {
          _id = el.$().attr("id");
          _node = $(`#${_id}`).get(0);
        } else if (el.jquery !== void 0) {
          _id = el.attr("id");
          _node = $(`#${_id}`).get(0);
        } else if (el.tagName !== void 0) {
          _id = el.id;
          _node = $(`#${_id}`).get(0);
        }
        return _node;
...
${js}
      ...
      getScriptSyncModeFromTextArea(jsLines);
      if (files.length === 0) {
        finalAppend($view, js, jsType);
      } else {
        yepnope({
          load: files,
          complete: () => {
  ...
  function finalAppend(container, js2, type, syncMode) {
    const script2 = document.createElement("script");
    script2.type = type;
    script2.className = "jsbox-script";
    script2.textContent = js2;
    document.body.appendChild(script2);
  ...
  function formatCode(code) {
    ...
    lines = lines.map((line) => {
      if (line.startsWith("//import")) {
        return line.replace("//import", pattern);
      } else if (line.startsWith("// import")) {
        return line.replace("// import", pattern);
      } else if (line.startsWith("//include")) {
        return line.replace("//include", pattern);
      } else if (line.startsWith("// include")) {
        return line.replace("// include", pattern);
      } else {
        return line;
